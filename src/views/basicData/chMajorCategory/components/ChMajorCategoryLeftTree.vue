<template>
  <a-card :bordered="false" style="height: 100%">
    <div class="j-table-operator" style="width: 100%">
      <a-button type="primary" preIcon="ant-design:plus-outlined" @click="onAddChMajorCategory">新增</a-button>
      <a-button type="primary" preIcon="ant-design:plus-outlined" @click="onAddChildChMajorCategory()">添加下级</a-button>
        ...
        >
          <template #title="{ key: treeKey, title, dataRef }">
            <a-dropdown :trigger="['contextmenu']">
              ...
              <template #overlay>
                <a-menu @click="">
                  <a-menu-item key="1" @click="onAddChildChMajorCategory(dataRef)">添加子级</a-menu-item>
    ...
    <ChMajorCategoryFormModal :rootTreeData="treeData" @register="registerModal" @success="loadRootTreeData" />
  </a-card>
</template>
...
<script lang="ts" setup>
  ...
  // 当前选中的中国专业分类
  const currentChMajorCategory = ref<any>(null);
  // 控制确认删除提示框是否显示
  ...
  async function loadRootTreeData() {
    try {
      loading.value = true;
      treeData.value = [];
      const result = await queryChMajorCategoryTreeSync();
      ...
      if (expandedKeys.value.length === 0) {
        ...
        if (selectedKeys.value.length === 0) {
          ...
          emit('select', currentChMajorCategory.value);
        }
      }
  ...
  function setSelectedKey(key: string, data?: object) {
    ...
    if (data) {
      currentChMajorCategory.value = data;
      emit('select', data);
    }
  }

  // 添加一级中国专业分类
  function onAddChMajorCategory() {
    openModal(true, { isUpdate: false, isChild: false });
  }

  // 添加子级中国专业分类
  function onAddChildChMajorCategory(data = currentChMajorCategory.value) {
    if (data == null) {
      createMessage.warning('请先选择一个专业分类');
      return;
    }
    const record = { pcode: data.code };
   ...
   */
  async function doDeleteChMajorCategory(idListRef, confirm = true) {
    const idList = unref(idListRef);
    if (idList.length > 0) {
      try {
        loading.value = true;
        await deleteBatchChMajorCategory({ ids: idList.join(',') }, confirm);
  ...
  async function onDelete(data) {
    if (data) {
      ...
      doDeleteChMajorCategory([data.id], false);
    }
  }
  ...
  async function onDeleteBatch() {
    try {
      await doDeleteChMajorCategory(checkedKeys);
      checkedKeys.value = [];
    } finally {
    }
  }
  ...
  function onExportXls() {
    handleExportXls('中国专业分类信息', Api.exportXlsUrl);
  }
