...
<script lang="ts" setup>
  ...
  const categoryOptions = computed(() => {
    if (props?.data?.pcode != '') {
      return chMajorCategoryTypeOptions.child;
    } else {
      return chMajorCategoryTypeOptions.root;
    }
  });
  ...
  onMounted(() => {
    // 禁用字段
    updateSchema([
      { field: 'pcode', componentProps: { disabled: true } },
  ...
  async function onSubmit() {
    try {
      ...
      //提交表单
      await saveOrUpdateChMajorCategory(values, isUpdate.value);
      //刷新列表
      emit('success');
